...
def calculateOverlapBetweenPredandExp(pred_df,exp_df,residue_overlap_frac_list):
    ...
    for residue_overlap_frac in residue_overlap_frac_list:
        for i,exp_peak in exp_df.iterrows():
            ...
            for j,pred_peak in pred_df.iterrows():
...
#     parser = PDBParser(QUIET=True)
#     df_list = []
#     for (gene,condition),pred_df_copy in filt_pred_df.groupby(['gene','condition']):
#         print(f"clustering {len(pred_df_copy)} predictions from {gene}_{condition} ...")
...
#         # Get the cluster representative for each cluster
#         pred_clusrep_df = pred_df_copy.loc[pred_df_copy.groupby('cluster')[pred_metric_name].idxmax()].sort_values(by='fragment start (aa)')
#         # get the first and last member for each cluster (to define the range of fragments that fall into it)
#         cluster_size = []
#         cluster_n_fragments = []
#         first_fragment_center = []
...
# slightly newer, but still not newest version...
# def generatePredictionsFromAlphaFold(pred_df: pd.DataFrame,
#                         pred_metric_name: str,
#                         pred_metric_cutoff: float,
#                         cluster_d_cutoff: float = 0.5,
...
#             last_fragment_center.append(cluster_n_df['fragment center (aa)'].max())
#         pred_clusrep_df['cluster_size'] = cluster_size
#         pred_clusrep_df['cluster_n_fragments'] = cluster_size
#         pred_clusrep_df['cluster first fragment center (aa)'] = first_fragment_center
#         pred_clusrep_df['cluster last fragment center (aa)'] = last_fragment_center
...
#             last_fragment_center.append(cluster_n_df['fragment center (aa)'].max())
#         pred_clusrep_df['cluster_size'] = cluster_size
#         pred_df_copy['cluster first fragment center (aa)'] = first_fragment_center
#         pred_df_copy['cluster last fragment center (aa)'] = first_fragment_center
